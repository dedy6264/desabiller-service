...
func ErrorCodeGateway(errCode string, state string) string {
	...
	if errCode != configs.WORKER_SUCCESS_CODE {
		if errCode == configs.WORKER_PENDING_CODE {
			return pending
		} else if ok, _ := InArray(errCode, []string{configs.WORKER_UNDEFINED_ERROR,
			configs.BILLER_DISRUPTION,
			configs.WORKER_BILL_NOTFOUND_CODE,
			configs.WORKER_FAILED_CODE,
			configs.WORKER_INVALID_PARAM, configs.WORKER_CREDENTIAL_ERROR, configs.WORKER_VALIDATION_ERROR, configs.WORKER_SYSTEM_ERROR}); ok {
			return failed
		}
	}
	return success
