...
func (svc providerServices) AddProvider(ctx echo.Context) error {
	...
	_, err = svc.services.ApiProduct.GetProvider(*req)
	if err != nil {
		if err == sql.ErrNoRows {
			...
			log.Println("Err ", svcName, "GetProvider", err)
			result := helpers.ResponseJSON(configs.FALSE_VALUE,
				configs.VALIDATE_ERROR_CODE,
				"failed",
				nil)
		...
		log.Println("Err ", svcName, "GetProvider", " Provider name is exist")
		result := helpers.ResponseJSON(configs.FALSE_VALUE,
			configs.VALIDATE_ERROR_CODE,
			"Provider name is exist",
			nil)
		return ctx.JSON(http.StatusOK, result)
	}
