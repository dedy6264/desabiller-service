...
func (svc nHierarchyMerchantOutletServices) DropMerchantOutletService(ctx echo.Context) error {
	var (
		svcName = "DropMerchantOutletService"
	...
	_, err = svc.services.ApiNHierarchy.NDropMerchantOutlet(req.ID)
	if err != nil {
		log.Println(logErr+"NDropMerchantOutlet", err.Error())
