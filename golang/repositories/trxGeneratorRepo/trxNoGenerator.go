...
func (ctx trxNoGenerator) GenerateNo(datatype string, prefix string, leadingZero ...int) (code string, err error) {
	...
	query := `select data_type, seqvalue from no_generator where data_type= $1`
	//cek data ada ga?
	...
	if err != nil {
		...
		//klo ga ada insert
		query := `insert into no_generator  (data_type,leadingzero)values($1,$2) returning data_type, leadingzero,prefix, seqvalue`
		...
		// update
		query = `update no_generator set data_type=$1,leadingzero=$2,seqvalue=$3 where data_type=$4`
